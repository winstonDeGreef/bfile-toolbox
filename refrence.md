...
## Server
The server to run the code on. This is most likely your local machine. Most of the time, the server will use the default port: `25632`.

However, if that port is not available, you will need to put the correct port in the server input. The port is shown in the terminal when you start the server. It will look something like this:

```Listening on http://localhost:<number>/```

Note: it does not matter if you use htpp:// or ws:
